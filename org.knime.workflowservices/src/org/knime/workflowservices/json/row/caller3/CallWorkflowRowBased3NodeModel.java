 ...
 */
public class CallWorkflowRowBased3NodeModel extends NodeModel {
    ...
    /**
     * @param nodeCreationConfiguration provides access to the optional file system connector that provides access to
     *            the location of the callee workflow
     */
    protected CallWorkflowRowBased3NodeModel(final NodeCreationConfiguration nodeCreationConfiguration) {
        super(getPortsConfig(nodeCreationConfiguration).getInputPorts(), getPortsConfig(nodeCreationConfiguration).getOutputPorts());
        m_configuration  = new CallWorkflowRowBased3Configuration(nodeCreationConfiguration);
    }
    ...
    protected BufferedDataTable[] execute(final PortObject[] inputs, final ExecutionContext exec) throws Exception {
        ...
        try {
            ...
            final var outputTable =
                KNIMEConstants.GLOBAL_THREAD_POOL.runInvisible(() -> executeInternal(getInputTable(inputs), exec));
    ...
    /**
     * @param inputs the connected input port objects
     * @return the port object that provides access to the data to process
     */
    private BufferedDataTable getInputTable(final PortObject[] inputs) {
        return (BufferedDataTable)inputs[CallWorkflowRowBased3NodeFactory.getDataPortIndex(m_configuration)];
    }
