...
final class WorkflowOutputNodeModel extends NodeModel implements OutputNode {
    ...
    public ExternalNodeData getExternalOutput() {
        final var outputPath = Optional.ofNullable(m_output);
        return m_settings.toExternalNodeData(getInPortType(0), outputPath.map(Path::toFile).orElse(null));
    }

    @Override
    protected void saveSettingsTo(final NodeSettingsWO settings) {
        NodeParametersUtil.saveParameters(WorkflowOutputSettings.class, m_settings, settings);
    }

    @Override
    protected void validateSettings(final NodeSettingsRO settings) throws InvalidSettingsException {
        var s = NodeParametersUtil.loadParameters(settings, WorkflowOutputSettings.class);
        WorkflowIOParameterNameValidation.validateParameterName(s.m_parameterName);
    }

    @Override
    protected void loadValidatedSettingsFrom(final NodeSettingsRO settings) throws InvalidSettingsException {
        m_settings = NodeParametersUtil.loadParameters(settings, WorkflowOutputSettings.class);
    }
