...
/**
 * Similar workaround using a checkbox to make the dialog dirty even if none of the controls in the dialog changed. It
 * does not use a third boolean that tracks whether it is initialized (which was only needed to prevent a very slight
 * flickering anyway) since in our scenario this checkbox is not necessarily only appearing initially but whenever the
 * condition that updates it becomes true).
 *
