...
class ExecutionParameters implements NodeParameters {
    ...
    enum InvocationDuration {
            @Label(value = "Long duration",
                description = "Choose this if the called workflow is expected to run for more than ten seconds. "
                    + "When selected, the invocation will poll its status repeatedly until the job completes (or fails). "
            ...
            @Label(value = "Short duration",
                description = "Choose this if the called workflow is expected to finish within ten seconds. "
                    + "When selected, the invocation will not poll its status but wait until the job completes (or fails). "
                    + "This removes the polling overhead and makes this option quicker for short-running workflows. "
                    + "Choosing this option for long-running workflows (> a minute) will cause timeout problems.")
            SHORT_DURATION;
    }

    @Widget(title = "Invocation duration", description = "Choose the expected duration of the called workflow execution.")
    @ValueSwitchWidget
    ...
    @Widget(title = "Discard job on successful execution",
        description = "When selected, successful jobs of the called workflow will be discarded from the remote executor. "
            + "When not selected, successful jobs will be kept.")
