 ...
 */
final class DialogSelectWorkflow extends JDialog {
     ...
     */
    private final transient Runnable m_triggerRefresh;
     ...
     * @param filteredWorkflowModel list of workflow paths
     */
    DialogSelectWorkflow(final Frame frame, final Consumer<String> setSelectedWorkflowPath,
        final Runnable triggerRefresh) {
        super(frame);

        m_setSelectedWorkflowPath = setSelectedWorkflowPath;
        m_triggerRefresh = triggerRefresh;
    ...
    void setWorkflowPaths(final List<String> paths) {
     ...
     */
    void open() {
        if (getState() == DialogSelectWorkflow.State.UNINITIALIZED) {
